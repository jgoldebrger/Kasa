...
import { downloadImportTemplate, downloadXlsxBlob, getImportColumns } from './import-templates'
...
describe('downloadImportTemplate (browser)', () => {
  ...
  it('downloads members and lifecycle-events templates with date and rtl columns', async () => {
    ...
    expect(
      (vi.mocked(document.createElement).mock.results[0]?.value as unknown as HTMLAnchorElement)
        .download,
    ).toBe('members-template.xlsx')

    await downloadImportTemplate('lifecycle-events')
    expect(
      (vi.mocked(document.createElement).mock.results.at(-1)?.value as unknown as HTMLAnchorElement)
        .download,
