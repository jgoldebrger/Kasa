...
export async function generateStatementPDF(
  ...
  try {
    ...
    const cityStateZip = [[lh.city, lh.state].filter(Boolean).join(', '), lh.zip]
      .filter(Boolean)
      .join(' ')
    ...
    yPosition -= 5
    addText(`Period: ${formatDate(statement.fromDate)} - ${formatDate(statement.toDate)}`, margin, {
      size: 10,
    })
    yPosition -= 20
    ...
    yPosition = balanceY
    addText(formatCurrency(statement.openingBalance), summaryRight, {
      size: 10,
      align: 'right',
      maxWidth: 100,
    })
    ...
    yPosition = incomeY
    addText(formatCurrency(statement.income), summaryRight, {
      size: 10,
    ...
    addText(formatCurrency(statement.withdrawals), summaryRight, {
      size: 10,
    ...
    if ((statement.cycleCharges || 0) > 0) {
      ...
      yPosition = cycleY
      addText(formatCurrency(statement.cycleCharges || 0), summaryRight, {
        size: 10,
        color: rgb(0.94, 0.27, 0.27),
        align: 'right',
        maxWidth: 100,
    ...
    yPosition = expensesY
    addText(formatCurrency(statement.expenses), summaryRight, {
      size: 10,
      color: rgb(0.94, 0.27, 0.27),
      align: 'right',
      maxWidth: 100,
    ...
    yPosition = closingY
    addText(formatCurrency(statement.closingBalance), summaryRight, {
      font: helveticaBoldFont,
      size: 12,
      align: 'right',
      maxWidth: 100,
    ...
    if (transactions.length > 0) {
      ...
      yPosition = tableTop
      addText('Description', tableLeft + dateWidth, {
        font: helveticaBoldFont,
        size: 9,
        maxWidth: descWidth,
      })
      yPosition = tableTop
      addText('Amount', tableLeft + dateWidth + descWidth, {
        font: helveticaBoldFont,
        size: 9,
        maxWidth: amountWidth,
      })
      yPosition = tableTop
      addText('Notes', tableLeft + dateWidth + descWidth + amountWidth, {
        font: helveticaBoldFont,
        size: 9,
        maxWidth: notesWidth,
      })
      ...
      transactions.forEach((transaction) => {
        ...
        const amountColor = transaction.amount >= 0 ? rgb(0.06, 0.73, 0.51) : rgb(0.94, 0.27, 0.27)
        yPosition = rowY
...
export async function generateTaxReceiptPDF(
  ...
  // Left: org block
  draw(org.name || '', margin, { font: helveticaBoldFont, size: 18, color: rgb(0.31, 0.27, 0.9) })
  ...
  const famCityStateZip = [[family.city, family.state].filter(Boolean).join(', '), family.zip]
    .filter(Boolean)
    .join(' ')
  ...
  // ---------------- Payments table ----------------
  draw('Contributions', margin, { font: helveticaBoldFont, size: 12, color: rgb(0.31, 0.27, 0.9) })
  yPosition -= 6
  const tableLeft = margin
  ...
  for (const p of payments) {
    ...
    page.drawText(formatDate(p.date), {
      x: dateCol,
      y: rowY,
      size: 9,
      font: helveticaFont,
    ...
    page.drawText(truncate(String(p.method || ''), 18), {
      x: methodCol,
      y: rowY,
      size: 9,
      font: helveticaFont,
    ...
    page.drawText(truncate(String(p.notes || ''), 30), {
      x: notesCol,
      y: rowY,
      size: 9,
      font: helveticaFont,
  ...
  page.drawText(totalLabel, {
    x:
      amountCol -
      helveticaBoldFont.widthOfTextAtSize(totalLabel, 11) +
      100 -
      helveticaBoldFont.widthOfTextAtSize(formatCurrency(runningTotal), 11) -
      20,
    y: yPosition,
    size: 11,
    font: helveticaBoldFont,
