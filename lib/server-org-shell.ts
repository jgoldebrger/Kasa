 ...
 */
export const loadServerOrgShell = cache(
  async (organizationId: string): Promise<ServerOrgShell | null> => {
    await connectDB()
    const org = await Organization.findById(organizationId)
      .select(
        'name slug currency locale branding.logoDataUrl branding.logoUpdatedAt branding.accentColor',
      )
      .lean<{
        name?: string
        slug?: string
