...
import { SavedPaymentMethod, Payment, Family, FamilyMember, RecurringPayment, Organization } from '@/lib/models'
...
export const POST = handler({
  ...
  fn: async ({ params, ctx, body, request }) => {
    ...
    const parentFamily = await Family.findOne({ _id: id, organizationId: ctx!.organizationId }).select('_id')
    if (!parentFamily) {
    ...
    const dayBucket = (paymentDate ? new Date(paymentDate) : new Date()).toISOString().slice(0, 16)
    ...
    if (ledgerWriteFailed) {
      console.error(
        '[charge-saved-card] ledger write failed after successful Stripe charge',
        {
          familyId: id,
