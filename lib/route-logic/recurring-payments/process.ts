...
import { sanitizeStripeErrorMessage, sanitizeBatchErrors } from '@/lib/payments/sanitize'
...
export const POST = handler({
  ...
  fn: async ({ ctx, request }) => {
    const rateVerdict = await checkRateLimit(request, 'recurring-process', {
      limit: 5,
      windowMs: 60_000,
    }, ctx!.organizationId)
...
export const GET = handler({
  ...
  fn: async ({ ctx, request }) => {
    ...
    const recurringPayments = await collectCompoundCursorPages<{
      ...
      (filter, limit) =>
        RecurringPayment.find(filter)
          ...
          .populate({
            ...
            select: 'last4 cardType expiryMonth expiryYear nameOnCard isDefault isActive organizationId',
            match: { organizationId: ctx!.organizationId },
          })
      ...
      1,
      (last) => ({
        v: last.nextPaymentDate
          ? new Date(last.nextPaymentDate as string | Date).getTime()
          : null,
        id: String(last._id),
      }),
    )
