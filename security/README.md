...
Reports land in `security/reports/output/zap/`. In GitHub Actions:

- **Weekly (Monday 07:00 UTC):** `security` workflow schedule runs Playwright (non-destructive) + ZAP **baseline**.
- **Manual:** trigger **security** workflow with `run_zap_full: true` for active scanning.
