    ...
    <div className="min-h-screen bg-app">
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <section className="text-center max-w-3xl mx-auto mb-20">
          ...
          <p className="text-sm text-fg-muted mt-4">{t('welcome.earlyAccess')}</p>
        </section>

        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-20">
          {FEATURES.map((feature) => (
            <div key={feature.titleKey} className="surface-card p-6">
              <h3 className="font-semibold text-fg mb-2">{t(feature.titleKey)}</h3>
              <p className="text-sm text-fg-muted leading-relaxed">{t(feature.bodyKey)}</p>
            </div>
          ))}
        </section>

        <section className="text-center surface-card p-10">
          <h2 className="text-2xl font-semibold text-fg mb-3">{t('welcome.cta.title')}</h2>
          <p className="text-fg-muted mb-6">{t('welcome.cta.subtitle')}</p>
